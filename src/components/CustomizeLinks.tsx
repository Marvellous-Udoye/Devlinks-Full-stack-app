...
export default function CustomizeLinks({ savedProfile, setSavedProfile }: CustomizeLinksProps) {
  ...
  const setIsSaveButtonActive = useCallback((isActive: boolean) => {
    setValidateCustomLinks((prevState) => ({ ...prevState, isSaveButtonActive: isActive }));
  }, []);

  const handleLinkChange = useCallback((index: number, field: string, value: string) => {
  ...
  }, [validateCustomLinks.links, validateCustomLinks.errors, setLinks, setErrors]);
  ...
  }, [validateCustomLinks.links, setLinks, setErrors, setIsSaveButtonActive]);

  const removeLink = useCallback((linkId: number) => {
    const updatedLinks = validateCustomLinks.links.filter((link) => link.id !== linkId);
    setLinks(updatedLinks);
    setErrors({});
  }, [validateCustomLinks.links, setLinks, setErrors]);

  const customStyles: StylesConfig<OptionType, false> = useMemo(() => ({
  ...
  const CustomOption = useCallback((props: any) => {
    return (
  ...
  const CustomSingleOption = useCallback((props: any) => {
    return (
  ...
  return (
    <div className="w-full bg-[#FAFAFA] px-6 pb-6 sm:px-4 sm:pb-4">
      <section className="bg-white p-10 flex flex-col gap-10 rounded-[12px] sm:p-4">
        <div className="custom_head">
          <p className="text-[32px] font-[700] leading-[48px] mb-[8px] sm:text-[24px]">Customize your links</p>
        ...
        <div className="grid grid-cols-1 gap-[24px]">
          ...
          {validateCustomLinks.links.length === 0 && (
            <div className="px-[20px] py-[44px] flex flex-col gap-10 bg-[#FAFAFA] rounded-[12px]">
              <svg className="mx-auto sm:w-[124.766px] sm:h-[80px]" xmlns="http://www.w3.org/2000/svg" width="250" height="161" viewBox="0 0 250 161" fill="none">
          ...
          {validateCustomLinks.links.map((link, index) => (
            <div className="flex flex-col gap-3 sm:p-4 p-[20px] bg-[#FAFAFA] rounded-[12px]" key={link.id}>
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-3">
                <div className="flex flex-col gap-1 relative">
                  ...
                  {validateCustomLinks.errors[index]?.platform && (
                    <span className="font-[400] text-[12px] text-[#FF3939] absolute right-0 -bottom-1/3 ">
                      {validateCustomLinks.errors[index].platform}
                    </span>
                  )}
                </div>

                <div className="relative flex flex-col w-full gap-1 mb-3">
                  <label
                    htmlFor={`link-${link.id}`}
                    className={`font-[400] text-[12px] text-[#333] ${validateCustomLinks.errors[index]?.url ? 'text-[#FF3939]' : ''}`}
                  >
                    Link
                  </label>
                  ...
                  <input
                    ...
                    className={`rounded-[8px] border pl-11 pr-4 py-3 focus:outline-none focus:shadow-custom-focus focus:border-[#633CFF] ${validateCustomLinks.errors[index]?.url ? 'border-[#FF3939]' : ''}`}
                  />
                  {validateCustomLinks.errors[index]?.url && (
                    <p className="font-[400] text-[12px] text-[#FF3939] absolute right-0 -bottom-1/3 ">
                      {validateCustomLinks.errors[index].url}
                    </p>
                  )}
                </div>
      ...
      <section className="py-[24px] px-[40px] flex justify-end border-t-[1px] border-t-[#D9D9D9] bg-white sm:p-4">
        <button
          type="button"
          onClick={handleSubmit}
          className={`py-[11px] px-[27px] text-white hover:shadow-custom-focus rounded-[8px] bg-[#633CFF] transition-opacity duration-300 ${validateCustomLinks.isSaveButtonActive || validateCustomLinks.links.length > 0 ? 'opacity-100' : 'opacity-25'} sm:w-full`}
        >
          Save
        </button>
      </section>
